...
<script>
  $(document).ready(function()
  {
    ...
    var actionCount = @if(old('actions')){{ count(old('actions')) }};@elseif(isset($monster->actions)) {{ $monster->actions->count() }}; @else 1; @endif

    var abilityTemplate = Hogan.compile($('#ability-template').html());
    var abilityCount = @if(old('abilities')){{ count(old('abilities')) }};@elseif(isset($monster->abilities)) {{ $monster->abilities->count() }}; @else 1; @endif
